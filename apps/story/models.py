...
class Chapter(models.Model):
    ...
    def get_available_choices(self, user):
        ...
        for choice in self.get_choices():
            if (
                user.money + choice.money_cost >= 0
                and user.energy + choice.energy_cost >= 0
            ):
...
class Choice(models.Model):
    ...
    from_chapter = models.ForeignKey(
        Chapter, related_name="choices_from", on_delete=models.CASCADE
    )
    to_chapter = models.ForeignKey(
        Chapter, related_name="choices_to", on_delete=models.CASCADE
    )
...
class ReaderUser(AbstractUser):
    ...
    groups = models.ManyToManyField(
        "auth.Group", related_name="reader_users", blank=True
    )
    user_permissions = models.ManyToManyField(
        "auth.Permission", related_name="reader_users", blank=True
    )
...
class ChoiceHistory(models.Model):
    ...
    user = models.ForeignKey(
        ReaderUser, related_name="choice_history", on_delete=models.CASCADE
    )
    choice = models.ForeignKey(Choice, on_delete=models.CASCADE)
    from_chapter = models.ForeignKey(
        Chapter, related_name="history_from", on_delete=models.CASCADE
    )
    to_chapter = models.ForeignKey(
        Chapter, related_name="history_to", on_delete=models.CASCADE
    )
