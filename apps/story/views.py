...
from .forms import LoginForm, ReaderUserCreationForm
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if not user.current_chapter:
                    starting_chapter = Chapter.objects.filter(
                        is_starting_chapter=True
                    ).first()
                    if starting_chapter:
...
def current_chapter_view(request):
    ...
    context = {
        ...
        "user_stats": {"money": user.money, "energy": user.energy},
...
def start_adventure_view(request):
    ...
    if not starting_chapter:
        messages.error(
            request, "No starting chapter found. Please contact the administrator."
        )
